...
from ExperienceReplay import Replay,SumTree
...
class prepState():
  ...
  def __init__(self):
    self.input_state = tf.placeholder(shape = [240,180,3], dtype = tf.float32)
    self.output = tf.image.rgb_to_grayscale(self.input_state)
    self.output = tf.image.crop_to_bounding_box(self.output, 50, 0, 190, 180)
    self.output = tf.image.resize_images(self.output,[84,84], method = tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    self.output = tf.squeeze(self.output)

  def process(self, sess, state):
    crop = state[75:75+240, 10:10+180, :]
...
class qNetwork():
  ...
  def _build_model(self):
    ...
    advantage_net = tf.contrib.layers.fully_connected(flattened, 512)
    value_net = tf.contrib.layers.fully_connected(flattened, 512)
...
def observeOne(sess, s, a, r, s_, d, qN, tN):
  p = np.expand_dims(s, axis=0)
  p_ = np.expand_dims(s_, axis=0)

  q_value = (qN.predict(sess, p))[0]
  q_value_next = (qN.predict(sess, p_))[0]
  q_target_next = (tN.predict(sess, p_))[0]
  ...
  if d == [True]:
    ...
    t[a] = r[0] + ri_dict["discount_factor"] * q_target_next[np.argmax(q_value_next)]

  error = abs(oldVal - t[a])
  return t, error

def observeBatch(sess, s, a, r, s_, d, qN, tN):
  q_values = qN.predict(sess, s)
  q_values_next = qN.predict(sess, s_)
  q_values_next_target = tN.predict(sess, s_)
  best_actions = np.argmax(q_values_next, axis = 1)
  targets_batch = r * (ri_dict["discount_factor"] * q_values_next_target[np.arange(ri_dict["batch_size"]), best_actions])
  # targets_batch = r + (np.invert(d).astype(np.float32) * ri_dict["discount_factor"] * q_values_next_target[np.arange(ri_dict["batch_size"]), best_actions])

  errors = np.zeros(ri_dict["batch_size"])

  for i in range(ri_dict["batch_size"]):
    t = q_values[i]
    oldVal = t[a[i]]
    errors[i] = abs(oldVal - targets_batch[i][a[i]])

  return q_values,targets_batch, errors
...
def deep_q_learning(sess, env,experiment_dir,qnetwork, targetNetwork, state_proc):
  ...
  replay_memory = Replay(ri_dict['replay_memory_size'])
  ...
  for i in range(ri_dict["replay_memory_init_size"]):
    ...
    if np.array_equal(next_state, [None]):
      ...
      replay_memory.add(reward[0], state, act, reward, next_state, done)
  ...
  for i in range(ri_dict["num_Episodes"]):
    ...
    for t in itertools.count():
      ...
      if np.array_equal(next_state, [None]):
        ...
        targetNS, error = observeOne(sess, state, act, reward, next_state,done, qnetwork, targetNetwork)
        #Save transition to replay memory
        replay_memory.add(error, state, act, reward, next_state, done)
      ...
      samples = replay_memory.sample(ri_dict["batch_size"])
      idx_batch, trans_batch = zip(*samples)
      states_batch, action_batch, reward_batch, next_states_batch, done_batch = map(np.array, zip(*trans_batch))

      #Calculate the q values and targets
      q_val, targets_batch, errors = observeBatch(sess, states_batch, action_batch, reward_batch, next_states_batch, done_batch, qnetwork, targetNetwork)

      for i in range(ri_dict["batch_size"]):
        idx = samples[i][0]
        replay_memory.update(idx, errors[i])
