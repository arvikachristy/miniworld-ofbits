...
\IEEEauthorblockN{\\ \large{University College London, Computer Science} }
}
...
% in the abstract
\begin{abstract}
Deep learning is an emerging field that has shown many breakthroughs recently due to the technology available. Within reinforcement learning, deep learning has shown to be effective in game-like environments. The purpose of this study is to investigate the effectiveness of using deep reinforcement learning to train agents on a novel benchmark, Mini World of Bits (developed by OpenAI). We look at adapting and evaluating deep Q-networks and policy gradient algorithms for this benchmark. We determine each model's success based off an average of the reward function from the environments, ideally looking towards a positive trend through the training process. [potential conclusion based on results]
\end{abstract}
...
This introduction will give the reader an insight into 

\subsection{OpenAI Universe}
Universe is a software platform built for training and measuring AI across a wide range of games, websites and other environments. It provides the agent's access to programs via a VNC client on a remote desktop, allowing the agent to perceive and act on the program the same way a human does: interpreting the pixel values of a screen and producing a combination of keyboard and/or mouse actions. The ultimate goal of universe is to develop a single agent that can apply all of its previous experiences to quickly adapt and master to other unfamiliar environments, which would demonstrate monumental progress towards general intelligence. This differs to other systems which typically fall into the category of "Narrow AI", which are systems that perform well in a specific domain but struggle to adapt to other environments.
