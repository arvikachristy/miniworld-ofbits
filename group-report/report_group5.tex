...
\\
One of the advantages of policy-based reinforcement learning technique is its ability to learn stochastic policies and has the potential to handle continuous action spaces. For the on-policy method, we make sure the agent learns how to create a policy by choosing the optimal solution over time. There are multiple methods that implement an on-policy approach, however we are going to focus on one of the widely-used approaches, policy gradient.
