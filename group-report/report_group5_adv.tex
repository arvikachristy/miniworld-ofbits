...
The main difference between these algorithms is whether they aim to explicitly capture the transition dynamics of the environment, or whether they approximate an action policy or an action-value function directly and thus sidestep the problem of uncertainty arising from an unknown environment.
...
However, the more complicated the environment, the harder it is to learn an accurate representation of it. Perhaps more widespread are model-free RL methods, which, instead of environment dynamics, iterate on either a value function or directly on the agent's policy instead.

For any successful RL agent, an important consideration is the balance between exploitation, i.e. extracting the maximum reward out of the environment according to current knowledge, and exploration, in which suboptimal actions are sampled to try to discover new ways to achieve even greater reward. A common strategy is epsilon-greedy, meaning that a random action is chosen with a probability of $\epsilon$, and the remaining actions are greedily sampled from the agent's current approximation. To enable convergence, $\epsilon$ is decayed over time from an initial value of 1 to a near-zero end value.
\subsection{Policy Gradient}
Policy gradient is an on-policy learning algorithm which trains a neural network to create a stochastic policy $\mu(a|s)$.
%by selecting optimal actions
%through gradient descent.
The policy determines a probability distribution over each action $a$ in the action space given a state $s$ of the environment.
% throughout the training process.
%In the perfect world,
The policy is iteratively refined using temporal difference (TD) learning and the network is trained using gradient descent.
The agent detects the wrong action by receiving a negative reward and the correct action by a positive reward. Throughout the training process, the chance of choosing the optimal action increases.
...
Among most notable variants of model-free RL is Q-learning, which learns an action-value function $Q^\pi(s, a)$, representing how much value the agent predict it will receive in the long run from taking action $a$ in state $s$ and following policy $\pi$ thereafter. For a time step $t$, $Q^\pi$ is defined as \[Q^\pi(s_t, a_t) = r_t + \gamma Q^\pi(s_{t+1}, a_{t+1})\] where $r_t$ is the reward received after taking action $a, s_{t+1}$ the state transitioned to, and $\gamma$ the discount factor [0, 1], which determines the importance of future rewards relative to current ones. The optimal action is selected by calculating the Q-value of each possible action in the current state, and choosing the highest value at a given state in time. This Q-function is updated at each time step in order to eventually converge to an optimal policy.

The agent is trained by a TD update on the Q-function:
\begin{align*}
...
\item \textbf{Experience Replay} : Before the training process, an agent stores its experiences during each time step \textit{e(t)} in the form of a tuple represented as (\({{S}}_{{t}}\), \({{A}}_{{t}}\), \({{R}}_{{t+1}}\), \({{S}}_{{t+1}}\), \(d\)), with \(d\) a boolean variable signifying if the step completes the episode. These tuples are stored in a replay memory \(D\) with a certain initial capacity. When the training process begins, the agent samples a batch of tuples from \(D\) that is fed onto a multi-layered neural network calculate predictions and update the Q-function for every \textit{e(t)}.
\end{itemize}

Both the inclusion of a target network and the randomization of the order of training on experiences assist with stabilizing the convergence of the Q-values \cite{deepqlearning}. When only a single network is used both for function approximation as well as target value generation or if current experiences are directly used in training, individual trajectories of the agent in the environment are more likely to influence the target values, leading to divergence in the long run.

\subsubsection{Dueling Deep Q-Learning}
Building on the DQN architecture, this approach splits the single output layer of the CNN into two streams, state-value and advantage, which are then re-combined to produce the final action-value function $Q(a,s) = V(s) + A(a | s)$. The advantage function $A(a | s)$ quantifies how much better than average it is to take action $a$ given state $s$. The rest of the Reinforcement Learning architecture remains intact.
...
\subsection{Supervised learning with reinforcement learning}
...
Due to time restriction, we had to be selective on choosing some of the MWoB environments that we were going to experiment with. There exist four types of environment such as click tasks, drag tasks, mouse tasks and other tasks. In this research, the main goal was to tackle the click tasks problems. Within the click tasks problem, we eliminated some of the environments that require Natural Language Processing such as \textit{"word-matching"} or \textit{"colour-matching"} as it would be more difficult to implement. At the end, as shown in figure \ref{fig:chosen-envi}, three environments were chosen; click the button, focus into the textbox, and close the dialog box.

All three environments have similar tasks that are achievable by creating a policy that clicks on the right area with the coordinate x and y as the 2D action space. Unfortunately, due to its early stage release, the environment source code has not been released yet by Open AI, which has consequently resulted in a restrictive exploration on the environment modification for this research. Therefore, the research is solely focusing on training the agents on the available environments using different models and techniques.
\begin{figure}
...
Figure \ref{fig:inputprocessing} presents three screen-shots that showcase some of these additions. The top left is a grayscaled and downsampled image of 84x84 pixels. Additionally, the key addition is that the last four frames before the current state are stacked into one image to provide directional information. The right image also uses a downsampled view of the canvas. This method however makes use of an additional field-of-view for the agent, or a sort of \textit{"second eyesight"} that has a magnified view of the cursor up-close that also follows its movement. The bottom-left presents an illustration for an implementation of edge detection using the SciKit-Image library. This edge data is embedded within the pixel information in hopes to identify features (buttons, checkboxes, etc.) within the canvas more efficiently. All these processing techniques have a general aim of providing an increased sense of supervision to the agent in hopes of speeding up the training process.
...
\paragraph{\bf Policy Gradient}
In this research, Vanilla Policy Gradient was chosen as the main on-policy approach. Vanilla PG is relatively easier to implement and suits the environment as it supports arbitrary number of actions. On top of that, the most important advantage of this approach is that it does not depend on the adjustment of the policy therefore more secure to be used for several policies \cite{Peters:2010}. However, in some cases a more filtered and chosen adjustment of the policy sometimes has the potential to perform better with a more complicated implementation.

It is quite challenging to fit Vanilla Policy Gradient without any support into the MWoB environment. Hence, visual manipulation to the initial input state had to be done such as cropping, transforming colour to greyscale and edge detection technique. At first, fully connected network was chosen as the policy, nonetheless the network has too much parameters and more difficult to train. Therefore, Convolutional Neural Network(CNN) was preferred as it considers neighbouring pixels to shrink down the calculation time and space.

To achieve the right input for the MWoB and Gym environment, TensorFlow played a crucial role to simplify and shaping the agent's input and output. Tensorflow was used to implement the CNN equivalence in much easier step such as expanding the network into additional layers, modifying input, and adding/updating functions. On top of that, the advantage of using TensorFlow is its ability to produce TensorBoard to track the agent's variable performance in a form of graph.

\subsubsection{Off-policy methods}  
An agent following an off-policy approach inherits the behavior of following the recommendation actions off of a second \(\epsilon\)-greedy policy, rather than an action the actual policy \(\pi\) might recommend it to take. This might result an agent to take actions of large consequences, but slowly learns to converge to an optimal policy over time. In this project, we decided to primarily look into adapting the Deep Q Network.\linebreak

\paragraph{\bf Deep Q-learning}
Considering the approach we took models a discrete action space, Deep Q Networks were adaptable with this environment. The inheritance of a Convolutional Neural Network (CNN) allowed us to easily feed the pixel data (along with the input processing techniques mentioned above) to the network and provide predictions for a fixed number of actions. As described earlier, mouse-based controls can be generalized to 5 discrete actions (with also an additional 6th for an agent to do nothing), and we decided to take this approach.\linebreak

There were however some slight implications when it came to adapting the Experience Replay technique and the hyper-parameters of the agent itself with the environment. There was the tendency within the Universe platform that would produce states with no pixel data at all (only a [None] state) if an episode were to complete when the time elapsed. This is known nature emitted by the environment to the Universe platform and there was the matter of how to deal with storing tuples if such transition arises, and if omitting this transition entirely would affect the training process of the network.\linebreak

Considering the nature of each episode within any environment being time-constraint and an adjustable frames per second factor, this also placed the problem of identifying optimal initial hyper-parameters of the network that would determine the extent of the training process (even to some extent, whether it would train in the first place). By the default, our environment ran at 15 frames per second (in turn, an agent can perform 15 actions per second). Within the scope of the 'Click-Test' environment, the worst-case for the number of steps per episode is 150 steps (due to the time constraint set to 10 seconds). This meant that episodes can finish rapidly, which meant that target network had to be updated rather quickly in order to avoid the convergence of predicted-values to 0, and required a much smaller set replay memory size for the Experience Replay process.\linebreak

\subsection{Reward manipulation}
One approach we looked into are additional reward manipulation techniques added to the existing reward signals in hopes to improve the efficiency of the training process. One caveat to these environments is that during the learning process, there is the possibility for a sequence of actions that can result in the cursor moving outside the canvas area onto irrelevant parts of the environment. While we can simply add logical expression prevent this behavior from happening, the agent would not directly learn from this restriction due to a lack of error signal of any sort. As a result, if the cursor is located at an edge and performs an action that would result in the cursor moving outside the canvas, it would receive a negative reward signal of -0.5. \linebreak

Inversely, we also try to provide positive reward signals if the cursor performed progressive steps towards completion of the task. For example, one of the environments involved clicking a randomly generated button within the canvas. One technique involved keeping record of the current position of the generated button, and set up a proximity area around the button. If the agent were to perform an action that would result in the position of the cursor to move closer towards the button, it would receive a positive reward signal of +0.1 for every step it did so. \linebreak

\subsection{Supervised methods}
Inspired by Inverse Reinforcement Learning, 
%TODO: cite
 we investigated approaches to imitation learning. By observing supervised optimal actions, the agent learns the reward function of the environment before learning the optimal policy to optimize for this reward function. While our method of sampling ideal actions was not necessarily scalable to other environments, it gave us evidence on whether the amount of positive episodes was a limiting factor in agent learning performance.

\subsection{Metrics}
To convey a complete picture, we ran each agent through a standard evaluation framework to ensure results between environments and training methods were consistent and could be compared with each other. The metrics we chose included actions per episode, percentage of successful episodes, and mean episode reward. The single most telling of these is, of course, mean reward. However, we felt that only looking at the mean reward does not accurately represent how much better the agent is performing if it is to be able to complete the task than if it did not manage to complete it at all within the 10 second time frame. A negative mean reward could, in fact, obscure the fact that as many as two thirds of the episodes were successful, as the relative weight of a late success is much smaller than that of a miss, -1.
...
For comparison, we included an agent which samples each action uniformly at random. For our agents to add value, they would need to outperform said random agent. Tables \ref{results-clicktest}, \ref{results-focustext} and \ref{results-closedialog}, demonstrate a performance improvement of x\% over the random baseline.
...
\subsubsection{Insufficient hardware}
The model combine convolutional neural network with reinforcement learning ,providing a good generalization of methodology against clicking task.However,it also requires longer training process to train the parameters in the model (the fully connected layers and convolution kernel of convolutional neural network ).We are currently training on machine with only one GPU and if using a machine even a cluster of machines with more GPUs(saying 10)  .our training process can achieve higher efficiency.

\subsubsection{Lack of environment source code}
Our agent learns under a model-free environment which means limited infromation about environment(only one reward and image of section where cursor can move) can be viewed .By taking advantage of source code of openai environment provided by andrej later,the agent  can fully know about the environment such as what kind of html components are  there and where they locate ,size of them and even  best policy in each step during training process thus  speed up training process by using reward manipulation.Also ,with help of model-based environment ,we can analyses training process by recording  and illustrating more critiria (not just reward). 
\subsubsection{unexceptedly runtime error }
Above these ,there are still some factors can potentially affect our training ,such as the error "reward falling behind" or frame loss .
