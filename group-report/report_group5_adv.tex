...
It is quite challenging to fit Vanilla Policy Gradient without any support into the MWoB environment. Hence, visual manipulation to the initial input state had to be done such as cropping, transforming colour to greyscale and edge detection technique. At first, fully connected network was chosen as the policy, nonetheless the network has too much parameters and more difficult to train. Fully connected network takes individual pixel as input resulting a large amount of data to be process however it analyses more complex data \cite{LongSD14}. To overcome this problem, CNN was chosen as it considers neighbouring pixels to shrink down the calculation time and space.
...
There were however some slight implications when it came to adapting the Experience Replay technique and the hyper-parameters of the agent itself with the environment. One implication was the tendency within the Universe platform that would produce states with no pixel data at all (only a [None] state) if an episode were to complete when the time elapsed. This is known nature emitted by the environment to the Universe platform and there was the matter of how to deal with storing tuples if such transition arises, and if omitting this transition entirely would affect the training process of the network. Another caveat we came across was experimentation with the \textit{Prioritized Experience Replay} process as explained earlier. The nature of calculating error-indices for tuples to be stored in the binary heap tree relied in the scope of positive-rewards only, which unfortunately for our circumstances, included negative rewards. This concluded to be an infeasible technique within our project \linebreak
