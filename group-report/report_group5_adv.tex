...
\\All three environments have similar tasks that are achievable by creating a policy that clicks on the right area with the coordinate x and y as the 2D action space. Unfortunately, due to its early stage release, the environment source code has not been released yet by Open AI. This results restrictive exploration on the environment modification for this research. Therefore, the research is focusing on training the agent itself on the available environment.
\begin{figure}
