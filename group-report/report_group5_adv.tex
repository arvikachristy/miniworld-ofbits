...
The policy is iteratively refined using temporal difference (TD) learning and gradient descent.
The agent detects the wrong action with negative reward and detects the correct action with positive reward. Throughout the training process, the chance of choosing the optimal action increases.
...
Q-learning can converge on optimal action-selection policy for a discrete MDP, however, it is less suitable than policy gradient methods to be adapted for continuous action domains.

\subsubsection{Deep Q-learning}
For an environment with discrete states, the Q-function can be implemented as a lookup table. However, the problem with naive Q-learning is that dynamic environments can exhibit a large scope of state-action pairs, and without any form of generalization, iterating through each possible state-action pair is highly impractical. There is also the caveat where slight changes to the Q-values can result in significant changes to the policy that can diverge the optimization of the Q-function. In such a case, neural networks can be used as a function approximator with weights \(\theta\) to help stabilize estimations of the action-value function. This form of agent is referred to as a \textbf{Deep Q-network}\cite{deepqlearning},
...
Figure \ref{fig:inputprocessing} presents three screenshots that showcases some of these experimented additions. The top left is a grayscaled and downsampled image to 84x84 pixels. Additionally, the key factor to this is that the last four frames before the current state are stacked into one image to provide directional information. The right image also uses a downsample view of the canvas. This method however makes use of an additional field-of-view, or a sort of \textit{"second eyesight"} that has a magnified view of the cursor up-close that also follows its movement. The bottom-right presents an illustration for an implementation of edge detection using the SciKit-Image library. This edge data is embedded within the pixel information in hopes to identify features (buttons, checkboxes, etc.) within the canvas more efficiently. All these processing techniques all have a general aim of providing an increased supervision to the agent in hopes of speeding up the training process.
...
\paragraph{\bf Q-learning}

\paragraph{\bf Deep Q-learning}

\paragraph{\bf Dueling Deep Q-learning}

\paragraph{\bf Double Deep Q-learning}
...
\subsection{Metrics}
To convey a complete picture, we ran each agent through a standard evaluation framework to ensure results between environments and training methods were consistent and could be compared with each other. The metrics we chose included actions per episode, percentage of successful episodes, and mean episode reward. The single most telling of these is, of course, mean reward. However, we felt that only looking at the mean reward does not accurately represent how much better the agent is performing if it is to be able to complete the task than if it did not manage to not complete it at all within the 10 second time frame. A negative mean reward could, in fact, obscure the fact that more than two thirds of the episodes were successful, as the relative weight of a late success is much smaller than that of a miss, -1.
...
To evaluate our models, we trained them for 3000 episodes on each of the three environments introduced in section 3.1. A typical learning process is depicted in Figure \ref{fig:tboard}. In 3000 episodes, the mean reward rises from around 0 to around 0.4. Any fluctuations in the training process are due to the inherent randomness of the environment, such as the distance from the cursor to the generated button, and the size of said button.  
...
\centering
	...
	\caption{A typical learning curve of our DQN agent on the ClickTest-v0 environment, plotted as a reward over episode and smoothed for clarity.}
	\label{fig:tboard}
\end{figure}
...
\subsubsection{Narrow action space}
MWOB has various categories of environments, such as dragging, clicking, typing. However, as what we suggested in 3.2 Action spaces, the task we currently focused on is to click a button and we expected the agent would gradually learn to move towards said button and finally click at the proper position. This agent is not applicable for dragging tasks, in which the agent would be asked to press and hold a component in the working area, as the action space of our agent is only [up, down, left, right, click, do nothing]. Other actions such as sliding or typing have not also not yet been considered.
\subsubsection{Complexity}
The task environments provided by MWOB have different complexities but the task we attempted to solve is comparatively simple. In some tasks, the agent needs to learn how to decide where to click among 3 possible buttons (as shown on the first task in figure \ref{fig:mwob})or even to choose an option from a dropdown menu according to the task prompt.
\subsubsection{Discrete action space}
So far we have designed a discrete action space for our model. It is not capable of dealing with tasks involving how long we press or hold a component or how far we move the cursor. These tasks could potentially be solved by using a Deep Deterministic Policy Gradient model (see future work).
\subsubsection{Amount of supervision}
The only reward which leads our agent to learn is the value obtained at the end of each episode (0 to 1 upon task completion or -1 for failure or timeout). By using reward manipulation, the agent would gain reward when approaching to button, giving a potentially faster learning rate.
