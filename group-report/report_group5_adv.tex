...
Unexpected results from some agents that scored relatively lower than desired may have been due to a training network that was not properly implemented. Every later was investigated and one of the most common possibilities is overfitting/underfitting the network. If this occurs, the network may not be able to generalise new data. It is very common in typical Neural Network implementation to miscalculate the perfect size of the network. Nevertheless, it is very difficult to know (at an early stage of training) the exact size of the network layer that can be deemed suitable for a particular application.
